...
import { BackgroundMapType, BaseMapLayerSettings, ColorDef } from "@itwin/core-common";
...
const BgMapWidget: FunctionComponent = () => {
  ...
  {viewport ? 
    <div>
        ...
        {/*'Solid Color' button*/}
        <Button onClick={()=>{
            viewport.view.displayStyle.backgroundMapBase = ColorDef.from(0,0,255, 100);
        }
        }>Solid Color</Button>
        </div>
        ...
        <Button onClick={()=>{
            viewport.view.displayStyle.changeBackgroundMapProvider(
              {name: "MapBoxProvider", type: BackgroundMapType.Aerial}
              );
        }
        }>MapBox Aerial</Button>
        </div>
        <div>
        ...
        <Button onClick={()=>{
            const value = viewport.view.displayStyle.backgroundMapSettings.useDepthBuffer;
            viewport.view.displayStyle.changeBackgroundMapProps({useDepthBuffer: !value});
        ...
        <Button onClick={()=>{
            ...
            viewport.view.displayStyle.changeBackgroundMapProps({applyTerrain: !value});
